...
const ACT_COLORS: Record<string, string> = {
  "Hero":           "rgba(99,102,241,0.85)",
  "Lifestyle":      "#C8860A",
  "Detail":         "#D4002A",
  "Scale":          "#1A8C42",
  "Flat Lay":       "rgba(139,92,246,0.85)",
        ...
        >
          {acts.map(([act, panels]) => {
            ...
            return (
              <div
                key={act}
                title={`${act} · shots ${panels[0]}–${panels[panels.length - 1]} (${panels.length}장)`}
...
export default function ChatPanel({
  ...
  // Generation complete
  const genCurrent = genProgress.current;
  const genTotal = genProgress.total;
  useEffect(() => {
    if (genDone) return;
    if (genTotal > 0 && genCurrent >= genTotal) {
      queueMicrotask(() => {
        setGenDone(true);
        setMessages((prev) => [
          ...prev,
          {
            id: nid(),
            role: "sys",
            text: `[${ts()}] complete · ${genCurrent}/${genTotal} shots`,
          },
          {
            id: nid(),
            role: "ai",
            text: `촬영이 완료되었습니다! ${genCurrent}장의 사진이 생성되었어요.\n사진을 클릭해서 선택한 후, 아래에 수정사항을 입력하세요.`,
            type: "text",
          },
    ...
    if (justFinished.length > 0) {
      const label =
        justFinished.length === 1
          ? `Shot #${justFinished[0]} updated`
          : `${justFinished.length} shots updated (#${justFinished.join(", #")})`;
      queueMicrotask(() => {
        setMessages((prev) => [
          ...prev,
          { id: nid(), role: "sys", text: `[${ts()}] ${label}` },
          {
            ...
            text:
              justFinished.length === 1
                ? `완료! #${justFinished[0]} 사진이 다시 생성되었어요.`
                : `완료! ${justFinished.length}장의 사진이 다시 생성되었어요. 더 수정할 사진을 선택하세요.`,
            type: "text",
          },
        ]);
      });
      ...
      setTimeout(() => {
        setMessages((prev) => [
          ...prev,
          {
            ...
            text:
              panelsToEdit.length === 1
                ? `${label} 사진을 다시 생성하고 있어요...`
                : `${panelsToEdit.length}장의 사진을 다시 생성하고 있어요 (${label})...`,
            type: "text",
          },
        ]);
      }, 150);
      ...
      setTimeout(() => {
        setMessages((prev) => [
          ...prev,
          {
            id: nid(),
            role: "ai",
            text: `먼저 사진을 선택하세요 (클릭 또는 Shift+클릭). 그 다음 수정사항을 입력하세요.`,
            type: "text",
          },
        ]);
      }, 150);
    ...
    <div className="chat-panel">
      ...
      <div className="phase-bar">
        {[
          ...
          { label: "SHOOT", done: !isGenerating && genDone },
        ].map((p, i) => (
